...
def parse_args():
    ...
    parser.add_argument('--template', default= 'qa', type=str)
...
print(f"save_path: {args.template}")
...
tokenize_instructions_fn = functools.partial(tokenize_instructions_qwen_chat, tokenizer=model.tokenizer, template='base')
tokenize_instructions_fn_qa = functools.partial(tokenize_instructions_qwen_chat, tokenizer=model.tokenizer, template='qa')
tokenize_instructions_fn_urial = functools.partial(tokenize_instructions_qwen_chat, tokenizer=model.tokenizer,template='urial')
...
harmless_cache_all = torch.zeros(args.N_INST_TRAIN,d_model,dtype=torch.float16)
harmless_cache_all_qa = torch.zeros(args.N_INST_TRAIN,d_model,dtype=torch.float16)
harmless_cache_all_urial = torch.zeros(args.N_INST_TRAIN,d_model,dtype=torch.float16)
...
for cur_id in tqdm(range(0, args.N_INST_TRAIN, args.batch_size)):
    ...
    print(f"extract token: {model.to_str_tokens(harmless_toks[0][args.pos])}")
    print(f"extract token qa: {model.to_str_tokens(harmless_toks_qa[0][args.pos])}")
    print(f"extract token urial: {model.to_str_tokens(harmless_toks_urial[0][args.pos])}")

    # run model on harmful and harmless instructions, caching intermediate activations
    _, harmless_cache = model.run_with_cache(harmless_toks,
                                           names_filter=lambda hook_name: f'blocks.{args.extract_layer}.hook_resid_pre' in hook_name,
                                           pos_slice=args.pos)
    print(f"harmless_cache:{harmless_cache[ f'blocks.{args.extract_layer}.hook_resid_pre'].shape}")
    harmless_cache_all[cur_id:cur_id + args.batch_size,:] = torch.squeeze(harmless_cache[ f'blocks.{args.extract_layer}.hook_resid_pre'])
    del harmless_cache
    gc.collect(); torch.cuda.empty_cache()

    _, harmless_cache_qa = model.run_with_cache(harmless_toks_qa,
                                                names_filter=lambda hook_name: f'blocks.{args.extract_layer}.hook_resid_pre' in hook_name,
                                                pos_slice=args.pos)
    harmless_cache_all_qa[cur_id:cur_id + args.batch_size,:] = torch.squeeze(harmless_cache_qa[ f'blocks.{args.extract_layer}.hook_resid_pre'])
    del harmless_cache_qa
    gc.collect(); torch.cuda.empty_cache()

    _, harmless_cache_urial = model.run_with_cache(harmless_toks_urial,
                                                   names_filter=lambda hook_name: f'blocks.{args.extract_layer}.hook_resid_pre' in hook_name,
                                                   pos_slice=args.pos)
    harmless_cache_all_urial[cur_id:cur_id + args.batch_size,:] = torch.squeeze(harmless_cache_urial[ f'blocks.{args.extract_layer}.hook_resid_pre'])
    del harmless_cache_urial
    gc.collect(); torch.cuda.empty_cache()
...
harmless_mean_act_qa = harmless_cache_all_qa.mean(dim=0)
harmless_mean_act_urial = harmless_cache_all_urial.mean(dim=0)
...
# only one layer
fwd_hooks = [(utils.get_act_name(act_name, args.extract_layer), hook_fn) for act_name in ['resid_pre', 'resid_mid', 'resid_post']]
...
intervention_generations = get_generations(model,
                                           harmless_inst_test[:args.N_INST_TEST],
