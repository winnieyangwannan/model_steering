...
for cur_id in tqdm(range(0, args.N_INST_TRAIN, args.batch_size)):
    ...
    harmless_logits, harmless_cache = model.run_with_cache(harmless_toks,
                                                           names_filter=lambda hook_name: f'blocks.{args.layer}.hook_resid_pre' in hook_name)
    ...
    for layer in range(model.cfg.n_layers):
        harmless_cache_all[cur_id:cur_id + args.batch_size,:] = harmless_cache[ f'blocks.{args.layer}.hook_resid_pre'][:, args.pos, :]
        # harmless_cache_all_qa[layer,cur_id:cur_id + args.batch_size,:] = harmless_cache_qa['resid_pre', layer][:, args.pos, :]
        harmless_cache_all_urial[cur_id:cur_id + args.batch_size,:] = harmless_cache_urial[ f'blocks.{args.layer}.hook_resid_pre'][:, args.pos, :]
