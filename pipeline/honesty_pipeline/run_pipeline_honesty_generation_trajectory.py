...
def generate_get_contrastive_activations_and_plot_pca(cfg, model_base, tokenize_fn, dataset, labels=None):
    ...
    # select one promt and visualize the trajectory
    ...
    with open(f'{cfg.artifact_path()}'+os.sep+'completions'+os.sep+f'{data_category}_completions_lying_trajectory.json', "w") as f:
        json.dump(activations_lying_trajectory, f)
    ...
    import pickle
    with open(artifact_dir + os.sep + model_name + '_' + 'activation_pca.pkl','wb') as f:
        pickle.dump(activation_pca)
